...
# --- Choix du client ---
client_id = st.number_input("Sélectionnez un client ID", min_value=1, step=1)

# Bouton pour charger les infos client
if st.button("Charger les infos client"):
    ...
    if response.status_code == 200:
        ...
        # Prédiction
        pred_response = requests.post(f"{API_URL}/prediction", json={"client_id": client_id})
        ...
        shap_local = requests.get(f"{API_URL}/local_feature_importance/{client_id}")
        ...
        shap_plot = requests.get(f"{API_URL}/shap_summary_plot/{client_id}")
...
if st.checkbox("Afficher l’importance globale des features"):
