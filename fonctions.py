...
def correlation_matrix(data, var_num, figsize=(8,6)):
    """
    ...
    sns.heatmap(correlation_matrix, cmap='coolwarm', mask=mask, annot=True, fmt='.2f', annot_kws={"size": 8})
...
def plot_stat(df, feature, title):
    """
    Affiche un countplot horizontal pour une variable catégorielle avec le pourcentage sur chaque barre.
    Chaque barre a une couleur différente.
    
    Args:
        df (DataFrame): DataFrame contenant la variable.
        feature (str): Nom de la colonne à analyser.
...
def plot_percent_target1(df, feature, title):
    """
    Affiche un barplot horizontal du pourcentage de TARGET=1 pour chaque modalité d'une variable catégorielle.
    Chaque barre a une couleur différente.
    
    Args:
        df (DataFrame): DataFrame contenant la variable et TARGET.
        feature (str): Nom de la colonne à analyser.
        title (str): Titre du graphique.
    """
    ...
    ax.set_title(title, fontsize=18)
    ax.set_xlabel("")
    ax.set_ylabel("Pourcentage de TARGET=1", fontsize=14)
